jQuery(document).ready(function($) {
    ...
    if (numRepeaterItems > 0) {
      ...
      if (numRepeaterItems > 1) {
        ...
        for (i = 1; i < numRepeaterItems; ++i) {
          upsiteAppendRow($(this), defaultValuesArray[i]);
        }
      }
    }
  });
  ...
  $(this).find('.sortable_repeater.sortable').sortable({
    update: function(event, ui) {
      upsiteGetAllInputs($(this).parent());
    }
  });
  ...
  $('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
    ...
    if (numItems > 1) {
      $(this).parent().slideUp('fast', function() {
        ...
        $(this).remove();
        upsiteGetAllInputs(parentContainer);
      })
    } else {
      $(this).parent().find('.repeater-input').val('');
      upsiteGetAllInputs($(this).parent().parent().parent());
    }
  });
  ...
  function upsiteAppendRow($element, defaultValue = '') {
  ...
  function upsiteGetAllInputs($element) {
  ...
  $('.multi-image-checkbox').on('change', function() {
    upsiteGetAllImageCheckboxes($(this).parent().parent());
  });

  // Get the values from the checkboxes and add to our hidden field
  function upsiteGetAllImageCheckboxes($element) {
  ...
  $(".pill_checkbox_control .sortable").sortable({
    ...
    update: function(event, ui) {
      upsiteGetAllPillCheckboxes($(this).parent());
    }
  });

  $('.pill_checkbox_control .sortable-pill-checkbox').on('change', function() {
    upsiteGetAllPillCheckboxes($(this).parent().parent().parent());
  });

  // Get the values from the checkboxes and add to our hidden field
  function upsiteGetAllPillCheckboxes($element) {
  ...
  $('.google-fonts-list').on('change', function() {
    ...
    var indexes = $.map(bodyfontcontrol.upsitefontslist, function(obj, index) {
    ...
    $.each(bodyfontcontrol.upsitefontslist[index].variants, function(val, text) {
    ...
    $(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.upsitefontslist[index].category);

    upsiteGetAllSelects($(this).parent().parent());
  });
  ...
  function upsiteGetAllSelects($element) {
    var selectedFont = {
  ...
  $('.customize-control-tinymce-editor').each(function() {
    ...
    var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].upsitetinymcetoolbar1;
    var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].upsitetinymcetoolbar2;
    var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].upsitemediabuttons;
 ...
 */
(function($, api) {
  api.sectionConstructor['upsite-upsell'] = api.Section.extend({
